...
# POST request to signin with credentials
def signin(usr, pwd):
...
def main():
    ...
    for _ in range(3): # Try to reauthenticate three times before giving up
        while True:
            try:
                ...
                if not zybook:
                    ...
                    while True:
                        ...
                        if zybook == i:
                            sys.exit(0)
                        elif zybook > i or zybook < 1:
                            print("Invalid zybook")
                            continue
                        else:
                            break

                book = books[int(zybook)-1]
                ...
                print(f'{chapters[-1]["number"] + 2}. [BACK]')
                selection = input("\nSelect a chapter: ")
                ...
                if selection == chapters[-1]["number"] + 2:
                    continue
                ...
                for i in range(0, sections[-1]["number"]):
                    solve_section(sections[i], code, chapter, auth)
